        ...
        function (height, callback) {
            ...
            async.doUntil(function (callback_until) {
                ...
                if (pool_hashrate_ratio < (1-stat_change_alert) || pool_hashrate_ratio > (1+stat_change_alert) ||
                    ...
                    global.support.sendtoTelegram(global.config.admin_telegram, 
...
function get_telegram_id(address, callback) {
    let currentTime = Date.now();
    if (!(address in minerTelegram_id) || currentTime - minerTelegram_id[address] > 10*60*1000) {
        minerTelegram_id[address] = currentTime;
