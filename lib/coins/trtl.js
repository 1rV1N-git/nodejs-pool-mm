...
function Coin(data){
    ...
    this.algoCheck = function(algos) {
        return algos.includes("cn-lite/1") || algos.includes("cn-pico/0") ?
            true : "algo array must include cn-lite/1 or cn-pico/0";
    }
    ...
    this.get_miner_agent_notification = function(agent) {
        ...
        if (m = reXMRig.exec(agent)) {
            ...
            if (majorv + minorv < 20700) {
                return "You must update your xmr-stak miner (" + agent + ") to v2.7.2+ (and use stellite in config)";
            }
