...
function Coin(data){
    ...
    this.algoCheck = function(algos) {
        return algos.includes("cn-lite/1") || algos.includes("cryptonight-lite/1") ?
			   true : "algo array must include cn-lite/1";
    }
    ...
    this.get_miner_agent_notification = function(agent) {
        ...
        if (m = reXMRig.exec(agent)) {
            ...
            if (majorv + minorv < 20300) {
                return "You must update your xmr-stak miner (" + agent + ") to v2.5.0+ (and use cryptonight_lite_v7 in config)";
            }
            ...
            if (majorv + minorv + minorv2 < 10607) {
                 return "You must update your SRBminer (" + agent + ") to version v1.6.7+";
            }
        }
        return false;
    };
