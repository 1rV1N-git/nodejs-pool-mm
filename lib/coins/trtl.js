...
function Coin(data){
    ...
    this.convertAlgosToCoinPerf = function(algos_perf) {
        ...
        if      ("cn-pico/trtl" in algos_perf)      coin_perf["TRTL"] = algos_perf["cn-pico/trtl"];
    ...
    this.get_miner_agent_notification = function(agent) {
        ...
        if (m = reXMRig.exec(agent)) {
            ...
            if (majorv + minorv < 20700) {
                return "You must update your xmr-stak miner (" + agent + ") to v2.7.2+ (and use turtlecoin in config)";
            }
