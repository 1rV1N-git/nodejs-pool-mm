...
function Coin(data){
    ...
    this.convertBlob = function(blobBuffer, port, childBlobBuffer){
			...
			console.log('blobBuffer '  + JSON.stringify(blobBuffer))
			console.log('port '  + port)
			console.log('childBlobBuffer '  + JSON.stringify(childBlobBuffer))

        return cnUtil.convert_blob(blobBuffer, this.portBlobType(port, blobBuffer[0]), childBlobBuffer);
    };

    this.constructNewBlob = function(blockTemplate, NonceBuffer, port, childBlockTemplateBuffer){
        return cnUtil.construct_block_blob(blockTemplate, NonceBuffer, this.portBlobType(port, blockTemplate[0]), childBlockTemplateBuffer);
    };
    ...
    this.BlockTemplate = function(template,childTemplate) {
        ...
        this.reserveOffset = template.reserved_offset + MERGE_MINING_TAG_RESERVED_SIZE - (POOL_NONCE_SIZE - 2); //mm
		console.log('reserved_offset ' + template.reserved_offset + ';reserveOffset ' + this.reserveOffset);
        ...
        this.nextBlob = function () {
			...
			if (mergedMining) {
				return global.coinFuncs.convertBlob(this.buffer, 0, childTemplate.buffer).toString('hex');
			}
