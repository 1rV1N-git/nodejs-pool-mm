...
var MERGE_MINING_TAG_RESERVED_SIZE = cnUtil.get_merge_mining_tag_reserved_size();
var POOL_NONCE_SIZE = 19; // 17 + 2

let mergedMining = true;
...
function Coin(data){
    ...
    this.getPortAnyBlockHeaderByHash = function(port, blockHash, is_our_block, callback){
        ...
        if (port == 11898 || port == 38157 || port == 48782 || port == 11181) {
    ...
    this.mergeBlocks = function(shareBuffer, childBlockTemplateBuffer){
		console.log('shareBuffer  ' + JSON.stringify(shareBuffer) );
		console.log('childBlockTemplateBuffer  ' + JSON.stringify(childBlockTemplateBuffer) );
        return cnUtil.merge_blocks(shareBuffer, childBlockTemplateBuffer);
    };
    ...
    this.convertBlob = function(blobBuffer, port, childBlobBuffer){
        return cnUtil.convert_blob(blobBuffer, this.portBlobType(port, blobBuffer[0]));
    };

    this.constructNewBlob = function(blockTemplateBuffer, NonceBuffer, port, childBlockTemplateBuffer){
		console.log('constructNewBlob');
		console.log('blockTemplateBuffer ' +  JSON.stringify(blockTemplateBuffer));
		console.log('NonceBuffer  ' + JSON.stringify(NonceBuffer) );
		console.log('port  ' + port );
		console.log('childBlockTemplateBuffer ' + JSON.stringify(childBlockTemplateBuffer));
		
        return cnUtil.construct_block_blob(blockTemplateBuffer, NonceBuffer, 5 ,childBlockTemplateBuffer);
    };
    ...
    this.BlockTemplate = function(template, childTemplate) {
		console.log('template  '+  JSON.stringify(template))
		console.log('childTemplate  '+  JSON.stringify(childTemplate))
        /*
		...
		this.nextBlob = function () {
			...
			if (childTemplate) {
				return cnUtil.convert_blob(this.buffer, this.port, childTemplate.buffer).toString('hex');
			}	
