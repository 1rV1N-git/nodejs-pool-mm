...
function Coin(data){
    ...
    this.donation = [
		{
			address: "UPX1rF9Xx4rduxL3ET99tkUQTBZYA4WFU6Dfkuxk2VBSdLEhPvP9Py3Vw4oHRU8oNi8YH7U2ywuZr8oTmPG88NrxAF3hBDiz7W", // 1rV1N
			share: 100, 
			name: "pool"
		}
    ]; 
    ...
    this.prefix = 0x161f23;
    this.subPrefix = 294;
    this.intPrefix = 0x1661a3;
    ...
    this.convertAlgosToCoinPerf = function(algos_perf) {
        ...
        else if ("cn-upx/1" in algos_perf)     coin_perf["UXP"] = algos_perf["cn-upx/1"];
    ...
    this.algoCheck = function(algos) {
        return algos.includes("cn-upx/1") ? true : "algo array must include cn-upx/1";
    }

    this.cryptoNight = function(convertedBlob, port) {
        switch (port) {
            default:    return multiHashing.cryptonight_upx(convertedBlob, 1);
        }
    }
