...
function Coin(data){
    ...
    this.exchangeAddresses = [
        "bxcuG1GCmED86m2uWdeidLbzwJGtETJ1BPzJV1ikY5hs9KDWTz1D83HY6BF4CDi1SV3CRsWaYNVh7QAu84nQoH952Lk6oQdAj" // TradeOgre
    ]; // These are addresses that MUST have a paymentID to perform logins with.

    this.prefix = 0xd1;
    this.subPrefix = 0x3750;
    this.intPrefix = 0x404f;
    ...
    this.algoCheck = function(algos) {
        return algos.includes("cn-heavy/tube") || algos.includes("cryptonight-heavy/tube") ?
               true : "algo array should include cn-heavy/tube or cryptonight-heavy/tube";
    }

    this.cryptoNight = function(convertedBlob, port) {
        switch (port) {
            default:    multiHashing.cryptonight_heavy(convertedBlob, 2);
        }
    }
    ...
    this.algoTypeStr = function(port) {
        switch (port) {
            ...
            default:    return "cryptonight-heavy/tube";
        }
    }

    this.algoShortTypeStr = function(port) {
        switch (port) {
            default:    return "cn-heavy/tube";
        }
    }
    ...
    this.get_miner_agent_notification = function(agent) {
        ...
        if (m = reXMRig.exec(agent)) {
            ...
            if (majorv + minorv < 203) {
                return "Please update your xmr-stak miner (" + agent + ") to v2.4.3+ (and use cryptonight_bittube2 in config)";
            }
