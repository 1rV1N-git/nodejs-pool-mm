...
function Coin(data){
    ...
    this.cryptoNight = function(convertedBlob) {
        return multiHashing.cryptonight(convertedBlob, convertedBlob[0] >= 4 ? 3 : 1);
    }

    this.blobTypeStr = function(blob) {
        switch (blob) {
            default:    return "cryptonote";
        }
    }

    this.algoTypeStr = function(blob) {
        switch (blob) {
            case "3":  return "cryptonight/1";
            default:    return "cryptonight/xtl";
        }
    }

    this.algoShortTypeStr = function(blob) {
        switch (blob) {
            case "3":  return "cn/1"
    ...
    this.variantValue = function(blob) {
        switch (blob) {
            case "3":  return 1;
            default:    return "xtl";
        }
    ...
    this.get_miner_agent_notification = function(agent) {
        ...
        if (m = reXMRig.exec(agent)) {
            ...
            if (majorv + minorv < 206) {
                return "Please update your XMRig miner (" + agent + ") to v2.6.2+";
            }
        } else if (m = reXMRSTAK.exec(agent)) {
            let majorv = parseInt(m[1]) * 100;
            let minorv = parseInt(m[2]);
            if (majorv + minorv < 203) {
                return "Please update your xmr-stak miner (" + agent + ") to v2.4.4+ (and use stellite in config)";
            }
    ...
    this.get_miner_agent_warning_notification = function(agent) {
        ...
        if (m = reXMRSTAK2.exec(agent)) {
            ...
            let minorv = parseInt(m[2]) * 100;
            let minorv2 = parseInt(m[3]);
            if (majorv + minorv + minorv2 < 20404) {
                return "Please update your xmr-stak miner (" + agent + ") to v2.4.4+ (and use stellite in config)";
            }
        } else if (m = reXNP.exec(agent)) {
            let majorv = parseInt(m[1]) * 10000;
