...
function Coin(data){
    ...
    this.algoCheck = function(algos) {
        return algos.includes("cn-heavy/0") || algos.includes("cryptonight-heavy/0") ?
               true : "algo array should include cn-heavy/0 or cryptonight-heavy/0";
    }

    this.cryptoNight = function(convertedBlob, port) {
        switch (port) {
            default:    return multiHashing.cryptonight_heavy(convertedBlob, 0); // LOKI
        }
    }

    this.blobTypeStr = function(port, version) {
        switch (port) {
            default: return version >= 9 ? "cryptonote_loki" : "cryptonote"; // LOKI
        }
    }

    this.algoTypeStr = function(port, version) {
        switch (port) {
            default:    return "cryptonight-heavy/0";
        }
    }

    this.algoShortTypeStr = function(port, version) {
        switch (port) {
            default:    return "cn-heavy/0";
        }
    }
