...
function Coin(data){
    ...
    this.getDefaultAlgos = function() {
        return [ "cn-heavy/0" , "cn-pico/trtl"];
    }

    this.getDefaultAlgosPerf = function() {
        return { "cn": 1, "cn-heavy/0": 1, "cn-pico/trtl": 1 };
    }
    ...
    this.cryptoNight = function(convertedBlob, port, height) {
        switch (port) {
            default:    return convertedBlob[0] >= 11 ? multiHashing.cryptonight_pico(convertedBlob, 0) : multiHashing.cryptonight_heavy(convertedBlob, 0);
        }
    }
    ...
    this.algoShortTypeStr = function(port, version) {
        switch (port) {
            default:    return version >= 11 ? "cn-pico/trtl" : "cn-heavy/0";    // LOKI
        }
    }
