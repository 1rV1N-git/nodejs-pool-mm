    ...
    global.mysql.query("SELECT * FROM config WHERE module = 'pool'").then(function (rows) {
        ...
        rows.forEach(function (row) {
            if (row.item == "targetTime" && parseInt(row.item_value) != global.config.pool.targetTime) {
                console.log("Changing targetTime time to: " + parseInt(row.item_value))
                global.config.pool.targetTime = parseInt(row.item_value);
            }
            if (row.item == "minerTimeout" && parseInt(row.item_value) != global.config.pool.minerTimeout) {
                global.config.pool.minerTimeout = parseInt(row.item_value);
                console.log("Changing minerTimeout to: " + parseInt(row.item_value))
            }
            if (row.item == "minDifficulty" && parseInt(row.item_value) != global.config.pool.minDifficulty) {
                ...
                console.log("Changing minDifficulty to: " + parseInt(row.item_value))
            }
            if (row.item == "maxDifficulty" && parseInt(row.item_value) != global.config.pool.maxDifficulty) {
                ...
                console.log("Changing maxDifficulty to: " + parseInt(row.item_value))
            }
            if (row.item == "buffer" && parseInt(row.item_value) != global.config.pool.buffer) {
                global.config.pool.buffer = parseInt(row.item_value);
                console.log("Changing buffer to: " + parseInt(row.item_value))
            }
       });
    });
}
