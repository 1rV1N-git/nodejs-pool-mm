...
let activeChildBlockTemplate; //MM 
let pastChildBlockTemplates = global.support.circularBuffer(10); //MM
...
function templateUpdateReal(coin, activePort, coinHashFactor, isHashFactorChange, childPort) {
	console.log('childPort ' + childPort)
	if (!childPort) {
    global.coinFuncs.getPortBlockTemplate(activePort, function (rpcResponse) {
        ...
        if (rpcResponse && typeof rpcResponse.result !== 'undefined') {
            ...
            console.log(threadName + "New block template found at " + rpcResponse.height + " height");
            if (cluster.isMaster) {
	...
	if (childPort){
		global.coinFuncs.getPortBlockTemplate(childPort, function (rpcResponse) {
			if (rpcResponse && typeof rpcResponse.result !== 'undefined') {
				rpcResponse = rpcResponse.result;
				rpcResponse.coin = coin;
				rpcResponse.port = childPort;
				rpcResponse.coinHashFactor = coinHashFactor;
				rpcResponse.isHashFactorChange = isHashFactorChange;
				console.log(threadName + "New child block template found at " + rpcResponse.height + " height");	
				
				activeChildBlockTemplate = new BlockTemplate(rpcResponse);
			} else {
				console.error("Block template request failed for " + childPort + " port.");
				setTimeout(templateUpdateReal, 3000, coin, activePort, coinHashFactor, isHashFactorChange, childPort);
			}
		});		
	}
}
    ...
    if (activePort && coinHashFactor) global.coinFuncs.getPortLastBlockHeader(activePort, function (err, body) {
        if (activePort !== global.config.daemon["activePort" + coin]) {
            ...
            if (!(coin in lastBlockHash) || body.hash !== lastBlockHash[coin]) {
				...
				templateUpdateReal(coin, activePort, coinHashFactor, isHashFactorChange, 11898);
...
function setNewBlockTemplate(template) {
	...
	console.log('activeChildBlockTemplate  ' + activeChildBlockTemplate)

    activeBlockTemplate[coin] = new BlockTemplate(template, (activeChildBlockTemplate ? activeChildBlockTemplate : null));
...
function getShareBuffer(miner, job, blockTemplate, params, childBlockTemplate) {
    ...
    if (!miner.proxy) {
        ...
        let shareBuffer = global.coinFuncs.constructNewBlob(template, new Buffer(nonce, 'hex'), blockTemplate.port, childBlockTemplate.buffer);

        return shareBuffer;
    
}

function processShare(miner, job, blockTemplate, params, childBlockTemplate) {
	//console.log('childBlockTemplate PS ' + JSON.stringify(childBlockTemplate));
    let hash;
    let shareType;
    let shareBuffer;
    ...
    if (global.config.pool.trustedMiners && miner.difficulty < 400000 && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 &&
		...
		console.log('getShareBuffer');
		console.log('blockTemplate ' + JSON.stringify(blockTemplate));
		console.log('childBlockTemplate ' + JSON.stringify(childBlockTemplate));
        shareBuffer = getShareBuffer(miner, job, blockTemplate, params, childBlockTemplate);
        if (shareBuffer === null) {
            process.send({type: 'invalidShare'});
  	    miner.sendNewJob();
            walletTrust[miner.payout] = 0;
...
console.log('childBlockTemplate.difficulty ' + childBlockTemplate.difficulty)

    if (hashDiff.ge(blockTemplate.difficulty*1000)) {
        ...
        function submit_block(retry) {
            global.support.rpcPortDaemon(blockTemplate.port, 'submitblock', [shareBuffer.toString('hex')], function (rpcResult) {
                if (rpcResult.error) {
					...
					templateUpdateReal('', blockTemplate.port, currCoinHashFactor[''], false);
					templateUpdateReal('', blockTemplate.port, currCoinHashFactor[''], false, 11898);
    ...
    } else if (hashDiff.ge(childBlockTemplate.difficulty)){
		
        // Submit block to the RPC Daemon.
        // Todo: Implement within the coins/<coin>.js file.
		shareBuffer = getShareBuffer(miner, job, blockTemplate, params, childBlockTemplate);
        let mergedBuffer = global.coinFuncs.mergeBlocks(shareBuffer, childBlockTemplate.buffer);
		
        function submit_block(retry) {
            global.support.rpcPortDaemon(childBlockTemplate.port, 'submitblock', [mergedBuffer.toString('hex')], function (rpcResult) {
                if (rpcResult.error) {
                    // Did not manage to submit a block.  Log and continue on.
                    recordShareData(miner, job, hashDiff.toString(), false, null, shareType);
                    let isNotifyAdmin = true;
                    if (shareType) {
                        let convertedBlob = global.coinFuncs.convertBlob(shareBuffer, childBlockTemplate.port);
                        hash = global.coinFuncs.cryptoNight(convertedBlob, childBlockTemplate.port, childBlockTemplate.height);
                        if (hash.toString('hex') !== resultHash) isNotifyAdmin = false;
                    }
                    console.error(threadName + "Error submitting block at height " + job.height + " (active block template height: " + activeBlockTemplate[childBlockTemplate.coin].height + ") from " + miner.logString + ", share type: " + shareType + ", valid: " + isNotifyAdmin + " error: " + JSON.stringify(rpcResult.error));
                    if (isNotifyAdmin) setTimeout(function() { // only alert if block height is not changed in the nearest time
                        global.coinFuncs.getPortLastBlockHeader(childBlockTemplate.port, function(err, body){
                            if (err !== null) {
                                console.error("Last block header request failed for " + childBlockTemplate.port + " port!");
                                return;
                            }
                            if (job.height == body.height + 1) global.support.sendEmail(global.config.general.adminEmail,
                                "FYI: Can't submit block to deamon on " + childBlockTemplate.port + " port",
                                "The pool server: " + global.config.hostname + " can't submit block to deamon on " + childBlockTemplate.port + " port\n" +
                                "Input: " + shareBuffer.toString('hex') + "\n" +
                                threadName + "Error submitting block at " + job.height + " height from " + miner.logString + ", share type: " + shareType + " error: " + JSON.stringify(rpcResult.error)
                            );
                       });
                    ...
                    //Success!  Submitted a block without an issue.
                    let blockFastHash = global.coinFuncs.getBlockID(shareBuffer, childBlockTemplate.port).toString('hex');
                    console.log(threadName + "Block " + blockFastHash.substr(0, 6) + " found at height " + job.height + " by " + miner.logString +
                        ", share type: " + shareType + " - submit result: " + JSON.stringify(rpcResult.result)
                    );
                    recordShareData(miner, job, hashDiff.toString(), true, blockFastHash, shareType, childBlockTemplate);
                } else {
                    if (retry) {
                        setTimeout(submit_block, 500, false);
                    } else {
...
function handleMinerData(method, params, ip, portData, sendReply, sendFinalReply, pushMessage) {
    ...
    switch (method) {
        ...
        case 'submit':
			...
			childBlockTemplate = activeChildBlockTemplate;/*
			console.log(JSON.stringify('pastChildBlockTemplates  ' + JSON.stringify(pastChildBlockTemplates)))
			console.log(JSON.stringify('activeChildBlockTemplate  ' + JSON.stringify(activeChildBlockTemplate)))
			console.log(JSON.stringify('childBlockTemplate  ' + JSON.stringify(childBlockTemplate)))*/

            job.rewarded_difficulty2 = job.rewarded_difficulty * job.coinHashFactor;

            let shareAccepted = processShare(miner, job, blockTemplate, params, childBlockTemplate);
