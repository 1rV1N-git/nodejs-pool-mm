...
let minerWallets = {};
const MAX_VER_SHARES_PER_SEC = 100; // per thread
...
function messageHandler(message) {
    switch (message.type) {
        ...
        case 'sendRemote':
            ...
            break;
        case 'sendChildRemote':
            if (cluster.isMaster) {
                global.database.sendChildQueue.push({body: Buffer.from(message.body, 'hex')});
            }
            break;
        case 'trustedShare':
            ++ trustedShares;
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, algos, algos_perf, algo_min_time) {
    ...
    this.childPaymentID = null;
    this.childCoin = "TRTL";
    this.childAddress = null;
	
    let diffSplit = login.split("+");
    let addressSplit = diffSplit[0].split(":")[0].split('.');
    let pass_split = pass.split(":");

    let addressChildSplit = diffSplit[0].split(":")
	if (addressChildSplit.length === 2) {
			addressChildSplit = addressChildSplit[1].split('.');
			this.childAddress = addressChildSplit[0]
	} else if (addressChildSplit.length.length > 2) {
        this.error = "Too many options in the login field";
        this.valid_miner = false;
        return;
    } else {
		if (global.coinFuncs.validateAddress(diffSplit[0].split('.')[0], true)){
			addressSplit = [global.coinFuncs.donationMergAddress,(diffSplit[0].split('.')[1] ? diffSplit[0].split('.')[1] : new Buffer(diffSplit[0].split('.')[0]).toString('hex').substring(0, 64))]
			this.childAddress = diffSplit[0].split('.')[0];
			this.childPaymentID = (diffSplit[0].split('.')[1] && diffSplit[0].split('.')[1].length === 64 && hexMatch.test(diffSplit[0].split('.')[1])) ?
				diffSplit[0].split('.')[1] : null;
		}else {
			this.childAddress = global.coinFuncs.donationMergeChildAddress;
		}
		
	}

    if (pass_split[0].split("@")[0].split('.')[0].length > 20 && global.coinFuncs.validateAddress(pass_split[0].split("@")[0].split('.')[0], true)){
		this.childAddress = pass_split[0].split("@")[0].split('.')[0];
		this.childPaymentID =  (pass_split[0].split("@")[0].split('.')[1] && pass_split[0].split("@")[0].split('.')[1].length === 64) ?
			pass_split[0].split("@")[0].split('.')[1] : null ;
		pass_split[0] = pass_split[0].split("@")[1] ? pass_split[0].split("@")[1] : 'worker';
	}
    ...
    if (typeof(addressChildSplit[1]) !== 'undefined') {
        if (addressChildSplit[1].length === 64 && hexMatch.test(addressChildSplit[1])) {
			this.childPaymentID = addressChildSplit[1];
		}else {
			this.error = "Invalid child paymentID provided";
			this.valid_miner = false;
			return;			
		}
	}
    ...
    if (!global.coinFuncs.validateAddress(this.childAddress, true)) {
		this.error = "Invalid child payment address provided: " + this.childAddress;
        this.valid_miner = false;
        return;
	}
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, isTrustedShare, blockTemplate) {
    ...
    if (blockCandidate) {
		if (global.config.daemon.port == blockTemplate.port) {
			global.database.storeShare(blockTemplate.height, global.protos.Share.encode({
			...
			global.database.storeChildShare(blockTemplate.height, global.protos.Share.encode({
				shares: job.rewarded_difficulty,
				shares2: job.rewarded_difficulty2, 
				paymentAddress: miner.childAddress,
				paymentID: miner.childPaymentID,
				foundBlock: blockCandidate,
				trustedShare: isTrustedShare,
				poolType: miner.poolTypeEnum,
				poolID: global.config.pool_id,
				...
				blockHeight: blockTemplate.height,
				timestamp: time_now,
				identifier: miner.childAddress == global.coinFuncs.donationMergeChildAddress ? ("Merge donation " + miner.childCoin) : miner.identifier,
				port: blockTemplate.port,
				share_num: 1
			}));
		}
	} else if (job.difficulty >= 1000000) {
		global.database.storeShare(blockTemplate.height, global.protos.Share.encode({
            shares: job.rewarded_difficulty,
            shares2: job.rewarded_difficulty2, 
            paymentAddress: miner.address,
		...
		global.database.storeChildShare(activeBlockTemplates[miner.childCoin].height, global.protos.Share.encode({
			shares: job.rewarded_difficulty,
			shares2: job.rewarded_difficulty2, 
			paymentAddress: miner.childAddress,
			paymentID: miner.childPaymentID,
			...
			poolType: miner.poolTypeEnum,
			poolID: global.config.pool_id,
			blockDiff: activeBlockTemplates[miner.childCoin].difficulty,
			bitcoin: miner.bitcoin,
			blockHeight: activeBlockTemplates[miner.childCoin].height,
			timestamp: time_now,
			identifier: miner.childAddress == global.coinFuncs.donationMergeChildAddress ? ("Merge donation " + miner.childCoin) : miner.identifier,
			port: activeBlockTemplates[miner.childCoin].port,
			share_num: 1
		}));
	} else {
        let wallet = walletAcc[wallet_key];
        ...
        if (height !== db_job_height || difficulty !== blockTemplate.difficulty || time_now - worker.time > 30*1000 || acc >= 1000000) {
            if (acc != 0) {
				...
				if (miner.childAddress && activeBlockTemplates[miner.childCoin]){
					global.database.storeChildShare(activeBlockTemplates[miner.childCoin].height, global.protos.Share.encode({
						shares: acc,
						shares2: acc2, 
						paymentAddress: miner.childAddress,
    ...
    if (is_walletAccFinalizer[wallet_key] === false) {
        ...
        setTimeout(walletAccFinalizer, 30*1000, wallet_key, miner.address, miner.paymentID, miner.bitcoin, miner.poolTypeEnum, blockTemplate.port);
    }
    ...
    if (blockCandidate) {
        if (global.config.daemon.port == blockTemplate.port) {
            ...
            global.database.storeChildBlock(blockTemplate.height, global.protos.Block.encode({
                hash: hashHex,
                difficulty: blockTemplate.difficulty,
                shares: 0,
                timestamp: time_now,
