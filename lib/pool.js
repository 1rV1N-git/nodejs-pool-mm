    ...
    global.mysql.query("SELECT * FROM config WHERE module = 'pool'").then(function (rows) {
        ...
        rows.forEach(function (row) {
            ...
            if (row.item == "minerTimeout" && parseInt(row.item_value) != global.config.pool.minerTimeout) {
                ...
                console.log(threadName + "Changing minerTimeout to: " + parseInt(row.item_value))
            }
            ...
            if (row.item == "buffer" && parseInt(row.item_value) != global.config.pool.buffer) {
                ...
                console.log(threadName + "Changing buffer to: " + parseInt(row.item_value))
            }
       });
    });
}
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate) {
    ...
    if (job.difficulty >= global.config.pool.buffer || blockCandidate) {
        ...
        if (height !== db_job_height || difficulty !== blockTemplate.difficulty || time_now - worker.time > 60*1000 || acc >= global.config.pool.buffer) {
            if (acc != 0) {
