...
function full_stop(err) {
    ...
    global.support.sendEmail(global.config.general.adminEmail, "Payment daemon unable to make payment BLOC",
    ...
    global.support.sendtoTelegram(global.config.admin_telegram, "Payment daemon unable to make payment BLOC");
}
...
let shapeshiftQueue = async.queue(function (task, callback) {
    ...
    async.waterfall([
        ...
        function (marketInfo, shapeshiftTxnData, body, intCallback) {
            // body.tx_hash = BLOC transaction hash.
...
let xmrToQueue = async.queue(function (task, callback) {
    ...
    async.waterfall([
        function (intCallback) {
            // Verify if BLOC.to is ready to get to work.
            xmrAPIClient.get('order_parameter_query/', function (err, res, body) {
                if (err) {
                    ...
                    console.log("Attempting to pay: " + address + " Amount: " + amtOfBTC + " BTC or " + amount / global.config.general.sigDivisor + " BLOC");
                    console.log("Response from BLOC.to: " + JSON.stringify(body));
                    if (body.lower_limit >= amtOfBTC) {
                        return intCallback("Not enough BLOC to hit the minimum deposit");
                    } else if (body.upper_limit <= amtOfBTC) {
                        return intCallback("Too much BLOC to pay out to xmr.to");
                    } else {
...
let paymentQueue = async.queue(function (paymentDetails, callback) {
    ...
    function getbalance() {
        global.support.rpcWallet("getBalance", {}, function (body) {
            ...
            console.log("Current wallet balance is " + global.support.coinToDecimal(body.result.lockedAmount + body.result.availableBalance) + " with " + global.support.coinToDecimal(body.result.availableBalance) + " unlocked balance");
            ...
            global.support.rpcWallet(transferFunc, paymentDetails, function (body) {
                ...
                if (body.hasOwnProperty('error') || !body.hasOwnProperty('result')) {
                    if (typeof(body.error) !== 'undefined' && body.error.hasOwnProperty('message') && body.error.message === "Wrong amount"){
    ...
    this.makePaymentWithID = function () {
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (typeof body.transactionHash !== 'undefined') {
                console.log("[*] Successful payment to " + identifier + " of " + global.support.coinToDecimal(amount) + " BLOC (fee " + global.support.coinToDecimal(fee) + " - " + global.support.coinToDecimal(global.config.payout.fee) + " = " + global.support.coinToDecimal(fee - global.config.payout.fee) + ") with tx_hash " + body.transactionHash.match(hexChars)[0]);
    ...
    this.trackPayment = function () {
        ...
        global.mysql.query("SELECT email FROM users WHERE username = ? AND enable_payout IS true limit 1", [payee.id]).then(function (rows) {
            ...
            global.support.sendEmail(rows[0].email,
                sprintf("Your %(payment_amount)s BLOC payment was just performed", emailData),
                sprintf(
                    "Your payment of %(payment_amount)s BLOC (with tx fee %(fee)s BLOC) to %(address)s wallet was just performed and total due was decreased by %(amount)s BLOC.\n" +
        ...
        global.mysql.query("SELECT telegram_id FROM users WHERE username = ? AND enable_tlg_payout IS true limit 1", [payee.id]).then(function (rows) {
            if (rows.length === 0) return;
			console.log("tlgid is " + rows[0].telegram_id );
            //if (rows[0].telegram_id === null) console.error("don't enter correct TLG ID of " + payee.id);return;
            // toAddress, subject, body
            let emailData = {
                address: payee.id,
                payment_amount: global.support.coinToDecimal(payee.amount - payee.fee),
    ...
    global.mysql.query("SELECT * FROM balance WHERE amount >= ?", [global.support.decimalToCoin(global.config.payout.walletMin)]).then(function (rows) {
        ...
        rows.forEach(function (row) {
            ...
            global.mysql.query("SELECT payout_threshold FROM users WHERE username = ?", [payee.id]).then(function (userRow) {
                roundCount += 1;
                let threshold = global.support.decimalToCoin(1);
                        ...
                        paymentQueue.unshift(paymentDetails, function (body) {  //jshint ignore:line
                            ...
                            if (typeof body.transactionHash !== 'undefined') {
                                ...
                                paymentDetails.transfers.forEach(function (payeeItem) {
                                    ...
                                    console.log("[**] Successful payment to " + payeeItem.address + " for " + global.support.coinToDecimal(payeeObjects[payeeItem.address].amount) + " BLOC (fee " + global.support.coinToDecimal(payeeObjects[payeeItem.address].fee) + ")");
                                });
                                console.log("[*] Successful payment to multiple miners of " + global.support.coinToDecimal(totalAmount) + " BLOC (fee " + global.support.coinToDecimal(totalFee) + " - " + global.support.coinToDecimal(global.config.payout.fee) + " = " + global.support.coinToDecimal(totalFee) + ") with tx_hash " + body.transactionHash.match(hexChars)[0]);
                                global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
