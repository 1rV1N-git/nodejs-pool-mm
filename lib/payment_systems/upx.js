...
function full_stop(err) {
    ...
    global.support.sendEmail(global.config.general.adminEmail, "Payment daemon unable to make payment UPX",
        ...
        function (intCallback) {
            ...
            xmrAPIClient.get('order_parameter_query/', function (err, res, body) {
                if (err) {
                    ...
                    console.log("Attempting to pay: " + address + " Amount: " + amtOfBTC + " BTC or " + amount / global.config.general.sigDivisor + " UPX");
...
function Payee(amount, address, paymentID, bitcoin) {
    ...
    this.makePaymentWithID = function () {
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (body.fee && body.fee > 10) {
                console.log("[*] Successful payment to " + identifier + " of " + global.support.coinToDecimal(amount) + " UPX (fee " + global.support.coinToDecimal(fee) + " - " + global.support.coinToDecimal(body.fee) + " = " + global.support.coinToDecimal(fee - body.fee) + ") with tx_hash " + body.tx_hash.match(hexChars)[0] + " and tx_key " + body.tx_key);
    ...
    this.trackPayment = function () {
        ...
        global.mysql.query("SELECT email FROM users WHERE username = ? AND enable_payout IS true limit 1", [payee.id]).then(function (rows) {
            ...
            global.support.sendEmail(rows[0].email,
                sprintf("Your %(payment_amount)s UPX payment was just performed", emailData),
                sprintf(
                    "Your payment of %(payment_amount)s UPX (with tx fee %(fee)s UPX) to %(address2)s wallet was just performed and total due was decreased by %(amount)s UPX.\n" +
        ...
        global.mysql.query("SELECT telegram_id FROM users WHERE username = ? AND enable_tlg_payout IS true limit 1", [payee.id]).then(function (rows) {
            ...
            global.support.sendtoTelegram(rows[0].telegram_id, sprintf(
                    "Your payment of %(payment_amount)s UPX (with tx fee %(fee)s UPX) to %(address)s wallet was just performed and total due was decreased by %(amount)s UPX.\n" +
    ...
    global.mysql.query("SELECT * FROM balance WHERE amount >= ?", [global.support.decimalToCoin(global.config.payout.walletMin)]).then(function (rows) {
        ...
        async.eachSeries(rows, function(row, next) {
            ...
            global.mysql.query("SELECT payout_threshold FROM users WHERE username = ?", [payee.id]).then(function (userRow) {
                let threshold = global.support.decimalToCoin(50);
                ...
                paymentQueue.unshift(paymentDetails, function (body) {  //jshint ignore:line
                    ...
                    if (body.fee && body.fee > 10) {
                        ...
                        paymentDetails.destinations.forEach(function (payeeItem) {
                            ...
                            console.log("[**] Successful payment to " + payeeItem.address + " for " + global.support.coinToDecimal(payeeObjects[payeeItem.address].amount) + " UPX (fee " + global.support.coinToDecimal(payeeObjects[payeeItem.address].fee) + ")");
                        });
                        console.log("[*] Successful payment to multiple miners of " + global.support.coinToDecimal(totalAmount) + " UPX (fee " + global.support.coinToDecimal(totalFee) + " - " + global.support.coinToDecimal(body.fee) + " = " + global.support.coinToDecimal(totalFee - body.fee) + ") with tx_hash " + body.tx_hash.match(hexChars)[0] + " and tx_key " + body.tx_key);
                        global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
