...
function full_stop(err) {
    ...
    global.support.sendEmail(global.config.general.adminEmail, "Payment daemon unable to make payment XTL",
    ...
    global.support.sendtoTelegram(global.config.admin_telegram, "Payment daemon unable to make payment XTL");
}
    ...
    this.trackPayment = function () {
        ...
        global.mysql.query("SELECT email FROM users WHERE username = ? AND enable_payout IS true limit 1", [payee.id]).then(function (rows) {
            ...
            global.support.sendEmail(rows[0].email,
                ...
                sprintf(
                    "Your payment of %(payment_amount)s XTL (with tx fee %(fee)s XTL) to %(address2)s wallet was just performed and total due was decreased by %(amount)s XTL.\n" +
                    (payee.tx_hash && payee.tx_key ?
                        "Your payment tx_hash (tx_id) is %(tx_hash)s and tx_key is %(tx_key)s (can be used to verify payment)\n" +
                        "Here is link to verify that this payment was made: https://explorer.stellite.cash/prove/%(tx_hash)s/%(address)s/%(tx_key)s\n" +
