        ...
        global.support.rpcWallet("getbalance", paymentDetails, function (body) {
            ...
            global.support.rpcWallet(transferFunc, paymentDetails, function (body) {
                ...
                if (body.hasOwnProperty('error') || !body.hasOwnProperty('result')) {
                    if (typeof(body.error) !== 'undefined' && body.error.hasOwnProperty('message') && (body.error.message === "Error while building Transaction err Insufficent unlocked balance" || body.error.message === "not enough unlocked money")){
    ...
    this.trackPayment = function () {
        ...
        global.mysql.query("SELECT email FROM users WHERE username = ? AND enable_payout IS true limit 1", [payee.id]).then(function (rows) {
            ...
            global.support.sendEmail(rows[0].email,
                ...
                sprintf(
                    "Your payment of %(payment_amount)s DERO (with tx fee %(fee)s DERO) to %(address2)s wallet was just performed and total due was decreased by %(amount)s DERO.\n",emailData),
                payee.id
            );
        });
