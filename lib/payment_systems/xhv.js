...
function full_stop(err) {
    ...
    global.support.sendEmail(global.config.general.adminEmail, "Payment daemon unable to make payment XHV",
        ...
        function (intCallback) {
            ...
            xmrAPIClient.get('order_parameter_query/', function (err, res, body) {
                if (err) {
                    ...
                    console.log("Attempting to pay: " + address + " Amount: " + amtOfBTC + " BTC or " + amount / global.config.general.sigDivisor + " XHV");
                    console.log("Response from XMR.to: " + JSON.stringify(body));
                    if (body.lower_limit >= amtOfBTC) {
                        return intCallback("Not enough XHV to hit the minimum deposit");
                    } else if (body.upper_limit <= amtOfBTC) {
                        return intCallback("Too much XHV to pay out to xmr.to");
                    } else {
...
function Payee(amount, address, paymentID, bitcoin) {
    ...
    this.makePaymentWithID = function () {
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (body.fee && body.fee > 10) {
                console.log("[*] Successful payment to " + identifier + " of " + global.support.coinToDecimal(amount) + " XHV (fee " + global.support.coinToDecimal(fee) + " - " + global.support.coinToDecimal(body.fee) + " = " + global.support.coinToDecimal(fee - body.fee) + ") with tx_hash " + body.tx_hash.match(hexChars)[0] + " and tx_key " + body.tx_key);
    ...
    this.trackPayment = function () {
        ...
        global.mysql.query("SELECT email FROM users WHERE username = ? AND enable_payout IS true limit 1", [payee.id]).then(function (rows) {
            ...
            global.support.sendEmail(rows[0].email,
                sprintf("Your %(payment_amount)s XHV payment was just performed", emailData),
                sprintf(
                    "Your payment of %(payment_amount)s XHV (with tx fee %(fee)s XHV) to %(address2)s wallet was just performed and total due was decreased by %(amount)s XHV.\n" +
                    (payee.tx_hash && payee.tx_key ?
                        "Your payment tx_hash (tx_id) is %(tx_hash)s and tx_key is %(tx_key)s (can be used to verify payment)\n" +
                        "Here is link to verify that this payment was made: https://explorer.havenprotocol.com/prove/%(tx_hash)s/%(address)s/%(tx_key)s\n" +
        ...
        global.mysql.query("SELECT telegram_id FROM users WHERE username = ? AND enable_tlg_payout IS true limit 1", [payee.id]).then(function (rows) {
            ...
            global.support.sendtoTelegram(rows[0].telegram_id, sprintf(
                    "Your payment of %(payment_amount)s XHV (with tx fee %(fee)s XHV) to %(address)s wallet was just performed and total due was decreased by %(amount)s XHV.\n" +
                ...
                paymentQueue.unshift(paymentDetails, function (body) {  //jshint ignore:line
                    ...
                    if (body.fee && body.fee > 10) {
                        ...
                        paymentDetails.destinations.forEach(function (payeeItem) {
                            ...
                            console.log("[**] Successful payment to " + payeeItem.address + " for " + global.support.coinToDecimal(payeeObjects[payeeItem.address].amount) + " XHV (fee " + global.support.coinToDecimal(payeeObjects[payeeItem.address].fee) + ")");
                        });
                        console.log("[*] Successful payment to multiple miners of " + global.support.coinToDecimal(totalAmount) + " XHV (fee " + global.support.coinToDecimal(totalFee) + " - " + global.support.coinToDecimal(body.fee) + " = " + global.support.coinToDecimal(totalFee - body.fee) + ") with tx_hash " + body.tx_hash.match(hexChars)[0] + " and tx_key " + body.tx_key);
                        global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
