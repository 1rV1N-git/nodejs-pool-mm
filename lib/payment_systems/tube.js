...
function full_stop(err) {
    ...
    global.support.sendEmail(global.config.general.adminEmail, "Payment daemon unable to make payment TUBE",
...
let shapeshiftQueue = async.queue(function (task, callback) {
    ...
    async.waterfall([
        ...
        function (marketInfo, shapeshiftTxnData, body, intCallback) {
            // body.tx_hash = TUBE transaction hash.
...
let xmrToQueue = async.queue(function (task, callback) {
    ...
    async.waterfall([
        function (intCallback) {
            // Verify if TUBE.to is ready to get to work.
            xmrAPIClient.get('order_parameter_query/', function (err, res, body) {
                if (err) {
                    ...
                    console.log("Attempting to pay: " + address + " Amount: " + amtOfBTC + " BTC or " + amount / global.config.general.sigDivisor + " TUBE");
                    console.log("Response from TUBE.to: " + JSON.stringify(body));
                    if (body.lower_limit >= amtOfBTC) {
                        return intCallback("Not enough TUBE to hit the minimum deposit");
                    } else if (body.upper_limit <= amtOfBTC) {
                        return intCallback("Too much TUBE to pay out to xmr.to");
                    } else {
...
function determineBestExchange() {
    async.waterfall([
        ...
        function (ssValue, callback) {
            xmrAPIClient.get('order_parameter_query/', function (err, res, body) {
                console.log("TUBE.to pricing body: " + JSON.stringify(body));
                if (err) {
                    return callback(err);
                } else if (body.error_msg) {
        ...
        debug("ShapeShift Value: " + global.support.bitcoinCoinToDecimal(ssValue) + " TUBE.to Value: " + global.support.bitcoinCoinToDecimal(xmrToValue));
        if (ssValue >= xmrToValue) {
            ...
            console.log("TUBE.to is the better BTC exchange, current rate: " + global.support.bitcoinCoinToDecimal(xmrToValue));
            bestExchange = 'xmrto';
...
function Payee(amount, address, paymentID, bitcoin) {
    ...
    this.makePaymentWithID = function () {
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (body.fee && body.fee > 10) {
                console.log("[*] Successful payment to " + identifier + " of " + global.support.coinToDecimal(amount) + " TUBE (fee " + global.support.coinToDecimal(fee) + " - " + global.support.coinToDecimal(body.fee) + " = " + global.support.coinToDecimal(fee - body.fee) + ") with tx_hash " + body.tx_hash.match(hexChars)[0] + " and tx_key " + body.tx_key);
    ...
    this.trackPayment = function () {
        ...
        global.mysql.query("SELECT email FROM users WHERE username = ? AND enable_payout IS true limit 1", [payee.id]).then(function (rows) {
            ...
            global.support.sendEmail(rows[0].email,
                sprintf("Your %(payment_amount)s TUBE payment was just performed", emailData),
                sprintf(
                    "Your payment of %(payment_amount)s TUBE (with tx fee %(fee)s TUBE) to %(address2)s wallet was just performed and total due was decreased by %(amount)s TUBE.\n" +
        ...
        global.mysql.query("SELECT telegram_id FROM users WHERE username = ? AND enable_tlg_payout IS true limit 1", [payee.id]).then(function (rows) {
            ...
            global.support.sendtoTelegram(rows[0].telegram_id, sprintf(
                    "Your payment of %(payment_amount)s TUBE (with tx fee %(fee)s TUBE) to %(address)s wallet was just performed and total due was decreased by %(amount)s TUBE.\n" +
                        ...
                        paymentQueue.unshift(paymentDetails, function (body) {  //jshint ignore:line
                            ...
                            if (body.fee && body.fee > 10) {
                                ...
                                paymentDetails.destinations.forEach(function (payeeItem) {
                                    ...
                                    console.log("[**] Successful payment to " + payeeItem.address + " for " + global.support.coinToDecimal(payeeObjects[payeeItem.address].amount) + " TUBE (fee " + global.support.coinToDecimal(payeeObjects[payeeItem.address].fee) + ")");
                                });
                                console.log("[*] Successful payment to multiple miners of " + global.support.coinToDecimal(totalAmount) + " TUBE (fee " + global.support.coinToDecimal(totalFee) + " - " + global.support.coinToDecimal(body.fee) + " = " + global.support.coinToDecimal(totalFee - body.fee) + ") with tx_hash " + body.tx_hash.match(hexChars)[0] + " and tx_key " + body.tx_key);
                                global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
