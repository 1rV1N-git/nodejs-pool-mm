...
function full_stop(err) {
    ...
    global.support.sendEmail(global.config.general.adminEmail, "Payment daemon unable to make payment GRFT",
    ...
    global.support.sendtoTelegram(global.config.admin_telegram, "Payment daemon unable to make payment GRFT");
}
        ...
        function (intCallback) {
            ...
            xmrAPIClient.get('order_parameter_query/', function (err, res, body) {
                if (err) {
                    ...
                    console.log("Response from GRFT.to: " + JSON.stringify(body));
    ...
    this.trackPayment = function () {
        ...
        global.mysql.query("SELECT email FROM users WHERE username = ? AND enable_payout IS true limit 1", [payee.id]).then(function (rows) {
            ...
            global.support.sendEmail(rows[0].email,
                ...
                sprintf(
                    "Your payment of %(payment_amount)s GRFT (with tx fee %(fee)s GRFT) to %(address2)s wallet was just performed and total due was decreased by %(amount)s GRFT.\n" +
        ...
        global.mysql.query("SELECT telegram_id FROM users WHERE username = ? AND enable_tlg_payout IS true limit 1", [payee.id]).then(function (rows) {
            ...
            global.support.sendtoTelegram(rows[0].telegram_id,
			    sprintf(
