...
let shapeshiftQueue = async.queue(function (task, callback) {
    ...
    async.waterfall([
        ...
        function (marketInfo, shapeshiftTxnData, body, intCallback) {
            // body.tx_hash = BSM transaction hash.
...
let xmrToQueue = async.queue(function (task, callback) {
    ...
    async.waterfall([
        function (intCallback) {
            // Verify if BSM.to is ready to get to work.
            xmrAPIClient.get('order_parameter_query/', function (err, res, body) {
                if (err) {
                    ...
                    console.log("Attempting to pay: " + address + " Amount: " + amtOfBTC + " BTC or " + amount / global.config.general.sigDivisor + " BSM");
                    console.log("Response from BSM.to: " + JSON.stringify(body));
                    if (body.lower_limit >= amtOfBTC) {
                        return intCallback("Not enough BSM to hit the minimum deposit");
                    } else if (body.upper_limit <= amtOfBTC) {
                        return intCallback("Too much BSM to pay out to xmr.to");
                    } else {
    ...
    this.makePaymentWithID = function () {
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (typeof body.transactionHash !== 'undefined') {
                console.log("[*] Successful payment to " + identifier + " of " + global.support.coinToDecimal(amount) + " BSM (fee " + global.support.coinToDecimal(fee) + " - " + global.support.coinToDecimal(global.config.payout.fee) + " = " + global.support.coinToDecimal(fee - global.config.payout.fee) + ") with tx_hash " + body.transactionHash.match(hexChars)[0]);
    ...
    this.trackPayment = function () {
        ...
        global.mysql.query("SELECT email FROM users WHERE username = ? AND enable_payout IS true limit 1", [payee.id]).then(function (rows) {
            ...
            global.support.sendEmail(rows[0].email,
                sprintf("Your %(payment_amount)s BSM payment was just performed", emailData),
                sprintf(
                    "Your payment of %(payment_amount)s BSM (with tx fee %(fee)s BSM) to %(address)s wallet was just performed and total due was decreased by %(amount)s BSM.\n" +
                        ...
                        paymentQueue.unshift(paymentDetails, function (body) {  //jshint ignore:line
                            ...
                            if (typeof body.transactionHash !== 'undefined') {
                                ...
                                paymentDetails.transfers.forEach(function (payeeItem) {
                                    ...
                                    console.log("[**] Successful payment to " + payeeItem.address + " for " + global.support.coinToDecimal(payeeObjects[payeeItem.address].amount) + " BSM (fee " + global.support.coinToDecimal(payeeObjects[payeeItem.address].fee) + ")");
                                });
                                console.log("[*] Successful payment to multiple miners of " + global.support.coinToDecimal(totalAmount) + " BSM (fee " + global.support.coinToDecimal(totalFee) + " - " + global.support.coinToDecimal(global.config.payout.fee) + " = " + global.support.coinToDecimal(totalFee) + ") with tx_hash " + body.transactionHash.match(hexChars)[0]);
                                global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
